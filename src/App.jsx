...
export default function KingdomOS() {
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", background: theme === "dark" ? "#030508" : "#ddd8cf", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <style>{FONTS}</style>
      ...
      <div style={{ width: 393, height: 852, borderRadius: 52, overflow: "hidden", position: "relative", background: T.bg, color: T.text, colorScheme: theme === "dark" ? "dark" : "light", boxShadow: "0 0 0 10px #111,0 0 0 11px #222,0 80px 140px rgba(0,0,0,.72)" }}>
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 52, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 28px 0", fontSize: 13, fontWeight: 700, zIndex: 20 }}><span>9:41</span><span style={{ fontSize: 11 }}>●●● WiFi 🔋</span></div>
        <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} style={{ position: "absolute", top: 60, right: 17, zIndex: 30, width: 30, height: 30, borderRadius: 99, background: T.surface, border: `1px solid ${T.border}` }}>{theme === "dark" ? "☀" : "☾"}</button>
        <main style={{ position: "absolute", top: 52, left: 0, right: 0, bottom: 82, overflowY: "auto" }} className="hs">
...
function BottomNav({ T, isDark, tab, setTab, setFabOpen, reminderCount }) {
  ...
  return (
    <div style={{ position: "absolute", left: 12, right: 12, bottom: 12, height: 62, borderRadius: 26, background: isDark ? "rgba(6,9,16,.88)" : "rgba(235,231,224,.92)", border: `1px solid ${isDark ? "rgba(255,255,255,.1)" : "rgba(0,0,0,.07)"}`, display: "flex", alignItems: "center", justifyContent: "space-around", backdropFilter: "blur(28px) saturate(1.8)", WebkitBackdropFilter: "blur(28px) saturate(1.8)", boxShadow: isDark ? "0 8px 32px rgba(0,0,0,.48), 0 1px 0 rgba(255,255,255,.06) inset" : "0 8px 24px rgba(0,0,0,.1), 0 1px 0 rgba(255,255,255,.9) inset" }}>
